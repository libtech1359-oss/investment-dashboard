...
ただし①の買付候補理由は【優先して使う根拠】（ATH乖離・Fear&Greed・ポートフォリオ比率・前回購入価格比）のルールを優先すること（候補選定ロジックは変更しない）。
...
【本日のリスク視点（機械選定・要約の導入で中心に据えること）】
コンテキストの「本日の部署別フォーカス視点」に指定された黒崎ミサキの視点を要約の切り口として使うこと。視点は自分で選ばず、指定された視点に従うこと。
視点がVIX・ドル円などコンテキストに実数値がある項目の場合はその数値を使うこと。
流動性・相関性・イベントリスク・急落耐性・分散効果など数値化しにくい視点の場合は、具体的な数値を捏造せず一般的なリスク構造として説明すること。
ただし要約の締めは従来通り、下記【数字で殴る部署にすること】ルールに従い集中投資率・現金比率・保有銘柄数のいずれか具体的な数字で結ぶこと（このルールは変更しない）。
...
□ 神谷シン・黒崎ミサキ・鬼塚ガイが「本日の部署別フォーカス視点」に指定された視点を実際に反映しているか
□ 秘書室長所見が「①今日の議論の本質→②学び→③重視した点→④今後の思想＋ブランドメッセージ」の順で書かれているか
□ 秘書室長所見に部署名・個人名を主語にした要約文（「神谷は〜」「黒崎は〜」等）が残っていないか
□ 「今日の見どころ」が実際の会議内容を反映した具体的なラベルになっているか（捏造していないか、「様子見継続」のような無内容なラベルでないか）
`.trim();
