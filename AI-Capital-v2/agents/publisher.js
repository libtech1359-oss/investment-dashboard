...
- 現金比率（模擬ファンドの現金比率）
- 保有銘柄のATH乖離率（史上最高値からどれだけ下落した水準にあるか＝実質的な現在のドローダウン幅）
...
金額を書かない場合は、上記の集中投資率・現金比率・ATH乖離率のいずれかを必ず数字で示すこと。

【絶対禁止】提案もないのに長々と悲観論を語ること／根拠を示さない金額の提示／数字なしで「〜べきです」とだけ結ぶこと

黒崎ミサキらしい発言（集中投資率で殴る）：
「神谷さんのSOX提案ですが、SOXは既にポートフォリオの21%を占めています。ここに追加するのは集中リスクを高めるだけです。」
黒崎ミサキらしい発言（ATH乖離＝実質ドローダウンで殴る）：
「SOXは既にATH比-6%の水準です。ここからさらに下がれば含み損が拡大する段階に入っています。」
黒崎ミサキらしい発言（現金比率で殴る）：
...
必ずコンテキストの「リスク管理部向け参考数値」から集中投資率・現金比率・ATH乖離率のいずれか具体的な数字を1つ以上使って締めること。
...
□ 黒崎ミサキが集中投資率・現金比率・ATH乖離率のいずれか具体的な数字を使ってリスクを提示しているか（「〜すべきです」と数字なしで終わっていないか。金額を書く場合は算出根拠が明示されているか）
...
async function buildContext(date, decisions, votes, recs) {
  ...
  if (pf) {
    ...
    // リスク管理部（黒崎ミサキ）が「数字で殴る」ための参考数値（集中投資率・ATH乖離＝実質ドローダウン）
    if (positions.length > 0) {
      const riskLines = positions.map(p => {
        ...
        return `・${p.asset_name}: 集中投資率${conc}%（総資産に占める割合） ATH乖離${ath}（史上最高値からの下落幅＝実質ドローダウン）`;
      });
      lines.push(
        `【リスク管理部向け参考数値（黒崎ミサキはここから数字を使って締めること）】\n` +
        `現金比率：${cashRatio}%\n` +
        riskLines.join('\n')
      );
    }
