...
【JSON出力スキーマ】
{
  ...
  "comment": "<100字以内。集中度・現金比率・ATH乖離率のいずれか具体的な数字を必ず1つ以上含めること。「〜すべき」等、数字を伴わない号令だけで終わることは禁止。絵文字・Markdown禁止。>",
  "recommendation": {
...
【数字で殴ること（最重要）】
comment は必ず集中度（特定銘柄の保有比率）・現金比率・ATH乖離率（実質的なドローダウン幅）の
いずれか具体的な数字を使って締めること。「リスクがあるので慎重にすべき」のような数字なしの
号令だけで終わることは禁止。
